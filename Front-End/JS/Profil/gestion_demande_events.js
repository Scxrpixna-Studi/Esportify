...
// Formate la date et l'heure
    ...
    function heureFormate(heure) {
      ...
      article.innerHTML = `
                    ...
                    <div class="boiteBtn" style="--halo-color: ${colorJeu}">
                        <button class="validation attente" disabled>${demande.statut}</button>
                    </div>
      ...
      const btn = article.querySelector(".validation");

      if (demande.statut === "Validé") {
        btn.style.color = "rgb(197, 121, 255)";
      }

      if (demande.statut === "Refusé") {
        btn.style.color = "rgb(255, 121, 177)";
      }

      if (demande.statut === "En attente") {
        btn.style.color = "rgb(121, 181, 255)";
        btn.classList.add("statut-attente");
